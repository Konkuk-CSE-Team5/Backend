...
public class VolunteerRecordCronService {
    ...
    public void updateExpiredPendingRecords() {
        ...
        try {
            // 1. 오늘 날짜의 PENDING 레코드 중 scheduledTime이 현재 시간보다 이전인 것들 조회
            List<VolunteerRecord> expiredTodayRecords = volunteerRecordRepository
                    .findPendingRecordsWithExpiredEndTime(today, nowTime);
            
            // 2. 실제 endTime이 지난 것들만 필터링 (서비스 레이어에서 계산)
            List<VolunteerRecord> actuallyExpiredTodayRecords = expiredTodayRecords.stream()
                    .filter(record -> {
                        // scheduledTime + (endTime - startTime) 계산
                        LocalTime scheduledTime = record.getScheduledTime();
                        LocalTime startTime = record.getScheduleDetail().getStartTime();
                        LocalTime endTime = record.getScheduleDetail().getEndTime();
                        
                        // Duration 계산
                        java.time.Duration duration = java.time.Duration.between(startTime, endTime);
                        LocalTime calculatedEndTime = scheduledTime.plus(duration);
                        
                        return calculatedEndTime.isBefore(nowTime);
                    })
                    .toList();
            
            // 3. 과거 날짜의 PENDING 레코드들 조회
            ...
            allExpiredRecords.addAll(actuallyExpiredTodayRecords);
    ...
    public int manuallyUpdateExpiredPendingRecords() {
        ...
        // 오늘 날짜의 PENDING 레코드 중 scheduledTime이 현재 시간보다 이전인 것들 조회
        List<VolunteerRecord> expiredTodayRecords = volunteerRecordRepository
                .findPendingRecordsWithExpiredEndTime(today, nowTime);
        
        // 실제 endTime이 지난 것들만 필터링 (서비스 레이어에서 계산)
        List<VolunteerRecord> actuallyExpiredTodayRecords = expiredTodayRecords.stream()
                .filter(record -> {
                    // scheduledTime + (endTime - startTime) 계산
                    LocalTime scheduledTime = record.getScheduledTime();
